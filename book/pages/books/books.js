...
Page({
  ...
  femaledetail(e) {
    ...
    wx.navigateTo({
      url: `/pages/ranking/ranking?id=${id}&title=${title}`,
    })
  },
  ...
  female(e) {
    ...
    let index = e.currentTarget.dataset.index
    wx.navigateTo({
      url: `/pages/classification/classification?name=${name}&index=${index}&sex=female`,
    })
  },
  publish(e) {
    console.log(e)
    ...
    let index = e.currentTarget.dataset.index
    wx.navigateTo({
      url: `/pages/classification/classification?name=${name}&index=${index}&sex=publish`,
    })
  },
  /**
   * 生命周期函数--监听页面加载
